...
Internet of things (IoT) have induced different requirements to those of the
traditional client-server applications~\cite{ahmed2019fog}. Such requirements
are constricted in terms of:

\begin{itemize}

\item \textbf{Latency:} for example, VR and gaming can only tolerate
end-to-end response times (including network and computation delays) of 20 ms.

\item \textbf{Bandwidth:} for example, stream processing and IoT, requiring
the transfer of large data to the cloud to be processed.
...
end-user~\cite{sill2017standards}. This approach grantees a low
...
end-user is connected to the application resources using a Local Area Network
(LAN), and as a result, does not suffer an ISP bandwidth
...
This opens the door for multitude of optimization possibilities, for example,
using MORPHEMIC and application can be deployed in a hybrid fashion where the
latency- and bandwidth- components of the application are deployed on the
edge. While the latency-tolerant, data-intensive, or resource-intensive
...
Similar to BYON, it deploys a node using a ProActive node agent, however
focusing on a lightweight implementation.
...
\vskip 2em
\textbf{MORPHEMIC adapts resources according to your application need}
\vskip 2em
...
resources that deliver the best performance to your specific needs. Equally,
MORPHEMIC can leverage this capability to deliver a tailored proactive
adaptation based on the application's traffic.
